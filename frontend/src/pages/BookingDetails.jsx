...
import { Calendar, Clock, MapPin, Phone, User, Briefcase, IndianRupee, XCircle } from 'lucide-react';
...
const BookingDetails = () => {
  ...
  const handleCancel = async () => {
    ...
    try {
      setCancelling(true);
      const res = await bookingsAPI.updateStatus(bookingId, 'Cancelled', reason);
      setBooking(res.data.data);

      let msg = 'Booking cancelled successfully';
      if (res.data.data?.cancellationPenalty?.applied) {
        msg += ` | Penalty: ₹${res.data.data.cancellationPenalty.amount}`;
      }
      toast.success(msg);
    } catch (e) {
          ...
          <motion.div initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} className="lg:col-span-2 space-y-6">
            <div className="card p-6">
              <div className="flex items-start justify-between gap-4">
                ...
                <span className={`badge ${booking.status === 'Completed' ? 'badge-success' : booking.status === 'In Progress' ? 'badge-warning' : booking.status === 'Cancelled' ? 'badge-error' : 'badge-info'}`}>
                  {statusLabel}
                </span>
              </div>
              ...
              {booking.status === 'Cancelled' && (
                <div className="mt-6 rounded-xl border-2 border-red-200 bg-red-50 p-4">
                  <div className="font-semibold text-red-700">Cancellation Details</div>
                  <div className="text-sm text-red-700 mt-2">
                    Reason: {booking.cancellationReason || 'Not provided'}
                  </div>
                  {booking.cancellationPenalty?.applied && (
                    <div className="text-sm text-red-700 mt-1">
                      Penalty: ₹{booking.cancellationPenalty.amount} ({booking.cancellationPenalty.reason})
                    </div>
                  )}
                </div>
              )}
            ...
            <div className="card p-6 space-y-3">
              ...
              {canCancel && (
                <button
                  ...
                  disabled={cancelling}
                  className="w-full rounded-xl border-2 border-red-200 text-red-600 hover:bg-red-50 py-3 font-semibold flex items-center justify-center gap-2 disabled:opacity-60"
                >
