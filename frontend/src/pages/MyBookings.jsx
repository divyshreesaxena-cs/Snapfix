...
import { Calendar, Star, IndianRupee, Clock, ChevronRight, XCircle } from 'lucide-react';
...
const MyBookings = () => {
  ...
  const handleCancelBooking = async (booking) => {
    ...
    try {
      ...
      const res = await bookingsAPI.updateStatus(booking._id, 'Cancelled', reason);
      const updated = res.data.data;

      let msg = 'Booking cancelled successfully';
      if (updated?.cancellationPenalty?.applied) {
        msg += ` | Penalty: ₹${updated.cancellationPenalty.amount}`;
      }
  ...
  const getStatusLabel = (booking) => {
    if (booking.status === 'Pending' && booking.workerStatus === 'Pending') {
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="space-y-2 text-sm text-dark-600">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <span>{Number(booking.worker.rating || 0).toFixed(1)} Rating</span>
                          ...
                          {booking.status === 'Cancelled' && booking.cancellationPenalty?.applied && (
                            <p className="text-red-600 font-medium mt-2">
                              Late cancellation penalty: ₹{booking.cancellationPenalty.amount}
                            </p>
                          )}
                        </div>
                      </div>
                    ...
                    <div className="flex flex-col gap-2 lg:w-56">
                      ...
                      {canCancel && (
                        <button
                          ...
                          className="w-full text-sm py-2 flex items-center justify-center gap-2 rounded-xl border-2 border-red-200 text-red-600 hover:bg-red-50 disabled:opacity-60"
                        >
