...
const WorkerSelection = () => {
  ...
  const { draft, setWorkerId } = useBookingDraft();
  ...
  const fetchWorkers = async () => {
    try {
      const pincodeToUse = draft?.address?.pincode || user?.pincode;
      ...
      <div className="min-h-screen flex items-center justify-center p-4">
        <div className="text-center">
          ...
          <p className="text-dark-600 mb-6">
            Sorry, no verified workers are available in this category near your location right now.
          </p>
  ...
  const typicalMin = rateInfo?.typicalRange?.min;
  const typicalMax = rateInfo?.typicalRange?.max;
  const recommended = rateInfo?.recommendedMedian;
  const policyMin = rateInfo?.policyBounds?.min;
  const policyMax = rateInfo?.policyBounds?.max;
            ...
            <div className="grid sm:grid-cols-3 gap-4 text-sm">
              <div>
                ...
                <p className="text-blue-900">
                  ₹{typicalMin} - ₹{typicalMax} / hr
                </p>
              </div>
              <div>
                <p className="text-blue-700 font-semibold">Recommended</p>
                <p className="text-blue-900">₹{recommended} / hr</p>
              </div>
              <div>
                <p className="text-blue-700 font-semibold">Policy Bounds</p>
                <p className="text-blue-900">
                  ₹{policyMin} - ₹{policyMax} / hr
                </p>
              </div>
            </div>
            {rateInfo?.source && (
              <p className="text-xs text-blue-700 mt-3">
                Pricing source: {rateInfo.source === 'live_workers' ? 'live worker rates' : 'benchmark estimate'}
              </p>
            )}
          </motion.div>
        )}
