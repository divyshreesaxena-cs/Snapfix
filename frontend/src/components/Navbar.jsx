...
import { Home, Calendar, LogOut, Menu, X, Wrench, ClipboardList, Users, Shield, UserCircle } from 'lucide-react';
...
const Navbar = () => {
  ...
  const navItems = role === 'worker'
    ? [
        ...
        { path: '/worker/profile-setup', label: 'Profile', icon: Calendar },
      ]
    : role === 'admin'
    ? [
        { path: '/admin/dashboard', label: 'Dashboard', icon: Shield },
        { path: '/admin/users', label: 'Users', icon: Users },
        { path: '/admin/workers', label: 'Workers', icon: Wrench },
  ...
  const homePath = role === 'worker' ? '/worker/dashboard' : role === 'admin' ? '/admin/dashboard' : '/home';
        ...
        <div className="flex justify-between items-center h-16">
          ...
          <Link to={homePath} className="flex items-center space-x-2 group">
            ...
            <div className="hidden sm:block">
              ...
              <div className="text-xs text-dark-500 -mt-1">{role === 'worker' ? 'Worker Portal' : role === 'admin' ? 'Admin Console' : 'Customer'}</div>
            </div>
          </Link>
          ...
          <div className="hidden md:flex items-center space-x-4">
            <div className="text-right">
              <p className="text-sm font-semibold text-dark-800">
                {role === 'worker' ? (user?.name || 'Worker') : role === 'admin' ? (user?.name || 'Admin') : (user?.fullName || 'User')}
              </p>
              <p className="text-xs text-dark-500">{role === 'admin' ? user?.email : user?.phone}</p>
            </div>
            <button
