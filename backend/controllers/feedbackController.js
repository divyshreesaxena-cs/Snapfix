...
const createFeedback = async (req, res) => {
  try {
    ...
    const completedPayment = await Payment.findOne({ booking: bookingId, paymentStatus: 'Completed' }).select('_id');
    if (!completedPayment) {
      return res.status(400).json({
        success: false,
        message: 'Payment must be completed before feedback can be submitted'
      });
    }
